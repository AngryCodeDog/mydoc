- [ ] 好像这里有个小问题，重启机器后没有在1935端口监听了！这个要注意一下

  原来卸载nginx时候没有卸载完全，你需要完整卸载，编译安装装在不同的地方，所以要设置一下nginx作为系统服务自动重启就可以了。

- [x] ffmpeg本地的还是有缺陷，暂时还没有动态设置地址的方法的。这个估计要写shell脚本了。

- [ ] 苹果摄像头地址居然是加密的，我拿不到摄像头地址，怎么破？

- [x] ffmpeg 命令没办法在后台运行？使用nohup commond & 让命令在后台运行就可以了
...
```
或者你可以build源码安装，参考官方提示：

另外可以参考：https://www.pyimagesearch.com/2016/10/24/ubuntu-16-04-how-to-install-opencv/

#### opencv+dlib人脸特征点

安装dlib。首先去官网（http://dlib.net/）下载dlib，解压后在根目录可以看到setup.py文件，在此目录下运行
...
我们可以采用opencv识别人脸框（但是识别有误差），然后用dlib检测出人脸特征点，再绘制出来即可。

```python
def get_face_point(ndarray_img):
    """ 检测人脸并绘制脸部矩形框 """
    ...
    for (x,y,w,h) in faces:
        ...
        # 人脸特征点 (通过坐标创建一个dlib.rectangle对象给dlib去检测)
